...
graph TD
    ...
    subgraph Actuators [Actuators & Modules]
        ...
        GreenLED[Green LED + 220Ω Resistor]
        RedLED[Red LED + 220Ω Resistor]
    end
    ...
    ESP32 -->|GPIO18 Output| GreenLED
    ESP32 -->|GPIO19 Output| RedLED
```
...
| **ESP32 NodeMCU** | `GPIO18` | **Green LED** | Anode (via Resistor $R_5$) | Status Indicator (ON = Normal) |
...
| **10** | 1 | `LED1` | Green LED indicator | Standard 5mm LED | Forward Voltage: 2.1V, Forward Current: 20mA max | Visual indicator for Alert/Normal system status (ON = Normal). |
| **11** | 1 | `LED2` | Red LED indicator | Standard 5mm LED | Forward Voltage: 2.0V, Forward Current: 20mA max | Visual indicator for warning/drowsy system status (ON = Drowsy). |
| **12** | 2 | `R5, R6` | Carbon Film Resistors | 220Ω Resistor | 220Ω, 0.25W, 5% tolerance | Limits LED current from GPIO pins to prevent thermal degradation. |
...
7. **LED Current-Limiting Resistors ($R_5, R_6$)**:
   - **Value**: $220\ \Omega$, $0.25\text{W}$, 5% tolerance.
   - **Placement**: Connected in series between ESP32 output pins (`GPIO18` and `GPIO19` respectively) and the anodes of the LEDs.
   - **Rationale**: Limits output current to a safe $\approx 6\text{mA}$ to prevent over-loading the ESP32 GPIOs and protect the LEDs from thermal stress.
...
| **Status LEDs** | 3.3V Out | 3.3V | 6 mA | 12 mA | 0.04 W |
...
### 6.2. Dual LED Status & Warning Indicator Circuit
Both the Green LED (Status) and Red LED (Warning) are powered directly by the ESP32's 3.3V GPIO pins (`GPIO18` and `GPIO19` respectively). Since the current draw is kept very low ($\sim 6\text{mA}$), they are driven directly without external transistors, using only current-limiting resistors.

```
       ESP32 Output
       GPIO18 (3.3V) ───[ 220Ω Resistor R5 ]───[ Anode (+) ] (Green LED) [ Cathode (-) ] ─── GND

       ESP32 Output
       GPIO19 (3.3V) ───[ 220Ω Resistor R6 ]───[ Anode (+) ] (Red LED)   [ Cathode (-) ] ─── GND
```
